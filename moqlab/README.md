...
publication time, and group-decision time. Grouping uses only summaries from the
latest completed receiver interval common to every ready client. Individual bottleneck-state and group
membership changes are also appended to `snapshots.jsonl.events.jsonl`. Empty
receiver intervals restart that flow's warm-up; feedback silence temporarily makes
the decision unknown without fabricating samples. Loss uses packet send times over
the same wall-clock evidence epoch as OWD.
