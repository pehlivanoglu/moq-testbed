...
def test_remove_pycaches_deletes_project_cache_dirs_and_bytecode(tmp_path: Path):
...
def test_remove_pycaches_deletes_pytest_cache(tmp_path: Path):
    root = tmp_path / "moqlab"
...
def test_remove_pycaches_skips_venv_and_runtime_dirs(tmp_path: Path):
    root = tmp_path / "moqlab"
    skipped_paths = [
